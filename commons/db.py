...
Cada servicio declara su DDL y hereda de ServicioBD, que le da la conexión con
commit/rollback y la creación idempotente de su tabla — a prueba de carreras
entre workers.

Las conexiones salen de un POOL compartido por todo el proceso
(psycopg2.pool.ThreadedConnectionPool): bajo concurrencia se reutilizan en vez de
abrir/cerrar una por operación.

POOL_MIN y POOL_MAX fijan el tamaño del pool POR WORKER; POOL_MAX × nº de workers
de gunicorn debe quedar por debajo de `max_connections` de PostgreSQL (default 100).
"""
import contextlib
import threading
